...
const personOne = new Person('Tim', 29);
console.log(personOne.toString());

personOne.eat('pizza');
personOne.eat('taco');
personOne.eat('hamburger');
...
/*
  TASK 2
    ...
    - xGive cars the ability to get fueled with a `.fill(gallons)` method. Add the gallons to `tank`.
...
class Car{
  ...
  const carOne =  new Car('toyota', 10)
...
const personTwo = new Lambdasian({ name: 'Tim', age: 29, location: 'Naples'});

// personTwo.speak();
/*
  TASK 4
    -x Write an Instructor class extending Lambdasian.
    - xIts constructor takes a single argument - an object with the following keys:
...
class Student extends Lambdasian{
  constructor(past){
    super(past);
    this.previousBackground = past.previousBackground;
    this.className = past.className;
    this.favSubjects = past.favSubjects;
  }
  listSubjects(){
    return `Loving ${this.favSubjects}`;
  }
  PRAssignment(task){
    return `${this.name} has submitted a PR for ${task}`
  }
  sprintChallenge(sprinting){
    return `${this.name} has begun sprint challenge on ${sprinting}`;
  }
...
class ProjectManager extends Instructor{
  constructor(past){
    super(past);
    this.gradClassName = past.gradClassName;
    this.favInstructor = past.favInstructor;
  }
  ...
  debugsCode(student, subject){
    this.student = student.student;
    this.subject = subject;
    return `${this.student} debugs ${student.name}'s code on ${this.subject}`;
  }
}
